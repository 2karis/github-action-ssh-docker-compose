...
if $DOCKER_SYSTEM_PRUNE ; then
  remote_command="set -e ; log() { echo '>> [remote]' \$@ ; } ; cleanup() { log 'Bring down all containers...' ; [ -d $HOME/workspace ] && docker compose -f $HOME/workspace/docker-compose.yaml down ; log 'Removing workspace...'; rm -rf \"\$HOME/workspace\" ; } ; log 'Creating workspace directory...' ; docker system prune -af"
fi
